...
import * as sandcastle from "file:///Users/jnsdls/Code/personal/watchtower/.sandcastle/.watchtower-sandcastle-94ea0517-0530-4840-a38a-ca824e9cc5cc.mjs";
...
// `bun install --frozen-lockfile` ensures the sandbox always has the exact
// dependencies recorded in bun.lock — no drift, no accidental upgrades.
const hooks = {
  sandbox: {
...
// Sandbox factory — every sandbox bind-mounts only the Codex files that must
// be shared. Keep auth.json writable so Codex can rotate short-lived access
// tokens, but leave skills/sessions/sqlite state container-local so parallel
// Codex agents do not race while installing system skills.
const dockerSandbox = () =>
  docker({
    mounts: [
      {
